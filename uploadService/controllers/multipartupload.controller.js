...
accessKeyId: process.env.AWS_ACCESS_KEY_ID,
 ...
 Body: req.file.buffer,  //buffer?? wt does it mean
 };
 const data = await s3.uploadPart(partParams).promise();
 console.log("data-------", data);
 res.status(200).json({ success: true }); //how are we extracting etag
 } catch (err) {
...
UploadId: uploadId,
 ...
 const parts = data.Parts.map(part => ({  // data.Parts.map || data.UplaodedParts.map
 ETag: part.ETag,
